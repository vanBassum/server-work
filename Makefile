...
up:
	@if [ -n "$(stack)" ]; then \
		...
		find $(CURDIR)/stacks -maxdepth 2 -type f -name "*-compose.yml" -execdir docker compose -f {} up -d \; ; \
	fi
...
down:
	@if [ -n "$(stack)" ]; then \
		...
		find $(CURDIR)/stacks -maxdepth 2 -type f -name "*-compose.yml" -execdir docker compose -f {} down \; ; \
	fi

pull:
	@echo "Updating all images..."
	@find $(CURDIR)/stacks -maxdepth 2 -type f -name "*-compose.yml" -execdir docker compose -f {} pull \;
